...
Notice how we assign the results of the function `sample()` of the `diamonds` data frame to a new data frame simply called `df`. Why? The data frame `df` is a [temporary variable](https://en.wikipedia.org/wiki/Temporary_variable) (aka [local variable](https://en.wikipedia.org/wiki/Local_variable) depending on [scope](https://en.wikipedia.org/wiki/Scope_(computer_science))). Thus, we can rerun the `sample()` function and overwrite the `df` data frame and reuse the output into the same visualization plot, i.e.:
...
Do the data points in the plot look different? How different would they look if we repeated this random sampling 10, 100, 1000 times?
